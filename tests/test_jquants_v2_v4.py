...
from quant_research.alpha_models.data.jquants_v2_v4 import adapters, collector as collector_module

from quant_research.alpha_models.data.jquants_v2_v4.collector import (
    CollectorFailure,
    HTTPResponse,
...
FREEZE_FILES = (
    "src/quant_research/alpha_models/data/jquants_v2_v4/__init__.py",
    "src/quant_research/alpha_models/data/jquants_v2_v4/contracts.py",
    "src/quant_research/alpha_models/data/jquants_v2_v4/collector.py",
    "src/quant_research/alpha_models/data/jquants_v2_v4/loader.py",
    "src/quant_research/alpha_models/data/jquants_v2_v4/adapters.py",
    "src/quant_research/alpha_models/data/jquants_v2_v4/launcher.ps1",
    "tests/test_jquants_v2_v4.py",
    "experiments/exp_20260828_005/artifacts/source_contract.json",
    "experiments/exp_20260828_005/artifacts/schema.json",
    "experiments/exp_20260828_005/parameters.json",
)
...
def _bar(day: str, code: str = "94330", *, factor: float = 1.0, ex_right: str | None = None, close: float = 100.0, market_cap: float | None = 1000.0) -> dict[str, object]:
    return {
        "Date": day,
        "Code": code,
        "O": close - 1.0,
...
def _calendar_object(day: str) -> CalendarDay:
    ...
    return CalendarDay(day, "1", policy, policy - 1, policy, "a" * 64)


def _master_object(day: str, code: str, sector: str) -> MasterRow:
    policy = policy_time_ms(day, 8, 0)
    raw_code = code + "0"
    return MasterRow(day, raw_code, code, f"Company {code}", f"Company {code}", sector, f"Sector {sector}", sector + "0", f"Sector33 {sector}", "TOPIX", "0111", "Prime", "1", "Margin", "011", policy, policy - 1, policy, "b" * 64)


def _bar_object(day: str, code: str, close: float) -> DailyBar:
    policy = policy_time_ms(day, 16, 30)
    return DailyBar(day, code + "0", code, close - 1, close + 1, close - 2, close, "0", "0", 1000.0, 100000.0, 1.0, close - 1, close + 1, close - 2, close, 1000.0, 1000.0, None, policy, policy - 1, policy, "c" * 64)


def _five_pages(*, q4_overlap: dict[str, object] | None = None) -> tuple[object, ...]:
    calendar = [{"Date": day, "HolDiv": "1" if day in ("2025-03-28", "2025-03-31") else "0"} for day in ("2025-03-28", "2025-03-29", "2025-03-30", "2025-03-31")]
    q4 = q4_overlap or _bar("2025-03-28", factor=0.5, ex_right="1")
    q5 = [_bar(day, factor=0.5 if day == "2025-03-28" else 1.0, ex_right="1" if day == "2025-03-28" else None) for day in EXPECTED_9433_DATES]
    payloads = (
        {"data": calendar},
        {"data": [_master()]},
        {"data": [_master()]},
...
class FiveQueryTransport:
    ...
    def __call__(self, url: str, api_key: str, cap_bytes: int) -> HTTPResponse:
        ...
        if parsed.path.endswith("calendar"):
            payload = {"data": [{"Date": day, "HolDiv": "1" if day in ("2025-03-28", "2025-03-31") else "0"} for day in ("2025-03-28", "2025-03-29", "2025-03-30", "2025-03-31")]}
            ...
            payload = {"data": [_bar(day, factor=0.5 if day == "2025-03-28" else 1.0, ex_right="1" if day == "2025-03-28" else None) for day in EXPECTED_9433_DATES]}
    ...
    def __call__(self, url: str, api_key: str, cap_bytes: int) -> HTTPResponse:
        ...
        if parsed.path.endswith("calendar"):
            ...
            if "pagination_key" not in query:
                return HTTPResponse(200, "application/json", _body({"data": [{"Date": "2025-03-28", "HolDiv": "1"}, {"Date": "2025-03-29", "HolDiv": "0"}], "pagination_key": "NEXT_PAGE"}), url)
            if query["pagination_key"] != ["NEXT_PAGE"]:
                raise AssertionError("pagination mutation")
            return HTTPResponse(200, "application/json", _body({"data": [{"Date": "2025-03-30", "HolDiv": "0"}, {"Date": "2025-03-31", "HolDiv": "1"}]}), url)
...
def _prepare_bound_repo(root: Path, overrides: dict[str, bytes] | None = None) -> str:
    ...
    manifest = {"files": sorted(entries, key=lambda row: row["path"].encode("utf-8")), "schema_version": "JQUANTS_V2_V4_EXTERNAL_FREEZE_V1"}
    target = root / "experiments/exp_20260828_005/artifacts/expected_freeze_manifest.json"
...
def _successful_temp_run(root: Path, transport: FiveQueryTransport | None = None) -> tuple[Path, Path, str]:
    ...
    parent = root / "data/raw/jquants_v2_v4/runs"
    return parent / "exp_20260828_005_formal_001", parent / ".exp_20260828_005_formal_001.control", expected
...
def _prepare_launcher_sandbox(root: Path, env_bytes: bytes | None, *, launcher_transform=None) -> tuple[Path, str, str]:  # type: ignore[no-untyped-def]
    ...
    overrides = {"src/quant_research/alpha_models/data/jquants_v2_v4/collector.py": fake_collector}
    ...
    launcher = root / "src/quant_research/alpha_models/data/jquants_v2_v4/launcher.ps1"
    freeze = root / "experiments/exp_20260828_005/artifacts/expected_freeze_manifest.json"
    control = root / "experiments/exp_20260828_005/formal_control"
...
def _attempt_paths(root: Path) -> tuple[Path, Path]:
    control = root / "experiments/exp_20260828_005/formal_control"
    return control / "exp_20260828_005_formal_001.reservation.lock", control / "exp_20260828_005_formal_001.stage_ledger.jsonl"
...
class JQuantsV2V4QueryContractTests(unittest.TestCase):
    ...
    def test_exp004_q01_raw_regression_and_official_holdiv_semantics(self) -> None:
        fixture = REPO_ROOT / "data/raw/jquants_v2_v3/runs/.exp_20260828_004_formal_001.staging/responses/01_Q01_CALENDAR_page_0001.json"
        if not fixture.is_file():
            self.skipTest("licensed exp004 raw fixture is not present")
        raw = fixture.read_bytes()
        self.assertEqual(sha256_bytes(raw), "97f544ad7fecdc2189383b5fdcef925b94914be00e2fd4a59aafb3d8cd63bdee")
        page = parse_page(QUERY_PLANS[0], page_number=1, status=200, body=raw, received_at_ms=RECEIVED)
        by_date = {row.session_date: row for row in page.records}
        self.assertTrue(by_date["2025-03-28"].is_session)
        self.assertFalse(by_date["2025-03-29"].is_session)
        self.assertFalse(by_date["2025-03-30"].is_session)
        self.assertTrue(by_date["2025-03-31"].is_session)
        half_day = CalendarDay("2025-01-02", "2", policy_time_ms("2025-01-02", 0, 0), 0, policy_time_ms("2025-01-02", 0, 0), "d" * 64)
        holiday_trading = CalendarDay("2025-01-03", "3", policy_time_ms("2025-01-03", 0, 0), 0, policy_time_ms("2025-01-03", 0, 0), "e" * 64)
        self.assertTrue(half_day.is_session)
        self.assertFalse(holiday_trading.is_session)

    def test_official_master_schema_product_category_and_old_alias_rejection(self) -> None:
        official = _master()
        self.assertEqual(set(official), set(MASTER_FIELDS))
        page = parse_page(QUERY_PLANS[1], page_number=1, status=200, body=_body({"data": [official]}), received_at_ms=RECEIVED)
        self.assertEqual(page.records[0].product_category, "011")  # type: ignore[union-attr]
        old = dict(official); old.pop("ProdCat"); old.update({"SecType": "1", "SecTypeNm": "Equity"})
        with self.assertRaisesRegex(ProbeError, "MASTER_FIELDS"):
            parse_page(QUERY_PLANS[1], page_number=1, status=200, body=_body({"data": [old]}), received_at_ms=RECEIVED)
        wrong = dict(official); wrong["ProdCat"] = "012"
        with self.assertRaisesRegex(ProbeError, "MASTER_NORMAL"):
            parse_page(QUERY_PLANS[1], page_number=1, status=200, body=_body({"data": [wrong]}), received_at_ms=RECEIVED)

    def test_official_free_bar_schema_nullable_fields_and_premium_key_rejection(self) -> None:
        ordinary = _bar("2025-03-28")
        self.assertEqual(set(ordinary), set(BAR_FIELDS))
        page = parse_page(QUERY_PLANS[3], page_number=1, status=200, body=_body({"data": [ordinary]}), received_at_ms=RECEIVED)
        row = page.records[0]; assert isinstance(row, DailyBar)
        self.assertEqual((row.ex_right_type, row.market_cap, row.upper_limit, row.lower_limit), (None, 1000.0, "0", "0"))
        for ex_right in ("1", "2", "3"):
            accepted = parse_page(QUERY_PLANS[3], page_number=1, status=200, body=_body({"data": [_bar("2025-03-28", ex_right=ex_right)]}), received_at_ms=RECEIVED)
            self.assertEqual(accepted.records[0].ex_right_type, ex_right)  # type: ignore[union-attr]
        premium = dict(ordinary); premium["MO"] = ordinary["O"]
        with self.assertRaisesRegex(ProbeError, "BAR_FIELDS"):
            parse_page(QUERY_PLANS[3], page_number=1, status=200, body=_body({"data": [premium]}), received_at_ms=RECEIVED)
        bad_limit = dict(ordinary); bad_limit["UL"] = "2"
        with self.assertRaisesRegex(ProbeError, "LIMIT_FLAG"):
            parse_page(QUERY_PLANS[3], page_number=1, status=200, body=_body({"data": [bad_limit]}), received_at_ms=RECEIVED)
        bad_cap = dict(ordinary); bad_cap["MktCap"] = -1
        with self.assertRaisesRegex(ProbeError, "MARKET_CAP"):
            parse_page(QUERY_PLANS[3], page_number=1, status=200, body=_body({"data": [bad_cap]}), received_at_ms=RECEIVED)
        rounded = dict(ordinary); rounded.update({"AdjO": 49.5, "AdjH": 50.5, "AdjL": 49.1, "AdjC": 50.0, "AdjVo": 2000.0})
        accepted = parse_page(QUERY_PLANS[3], page_number=1, status=200, body=_body({"data": [rounded]}), received_at_ms=RECEIVED)
        self.assertEqual(accepted.records[0].adjusted_low, 49.1)  # type: ignore[union-attr]
    ...
    def test_exrt_is_required_and_split_expectation_is_exact(self) -> None:
        ...
        self.assertEqual((split.adjustment_factor, split.ex_right_type), (0.5, "1"))
        bad = list(_five_pages())
        q5 = [_bar(day, factor=1.0, ex_right=None) for day in EXPECTED_9433_DATES]
    ...
    def test_injected_clock_paces_every_request_without_real_sleep(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp:
            staging = Path(temp)
            pages, receipts, _ = collect_pages(api_key="SYNTHETIC_KEY", transport=transport, utc_clock=clock.utc, monotonic=clock.monotonic, sleeper=clock.sleep, staging=staging)
            self.assertEqual(len(tuple((staging / "responses").glob("*.json"))), 5)
            self.assertEqual(len(tuple((staging / "response_receipts").glob("*.receipt.json"))), 5)
            for path in staging.rglob("*"):
                if path.is_file(): self.assertNotIn(b"SYNTHETIC_KEY", path.read_bytes())
    ...
    def test_raw_and_safe_receipt_survive_master_semantic_failure(self) -> None:
        class OldMasterTransport(FiveQueryTransport):
            def __call__(self, url: str, api_key: str, cap_bytes: int) -> HTTPResponse:
                if urlsplit(url).path.endswith("master"):
                    self.calls.append(url)
                    old = _master(); old.pop("ProdCat"); old.update({"SecType": "1", "SecTypeNm": "Equity"})
                    return HTTPResponse(200, "application/json", _body({"data": [old]}), url)
                return super().__call__(url, api_key, cap_bytes)
        with tempfile.TemporaryDirectory() as temp:
            staging=Path(temp); clock=VirtualClock(); transport=OldMasterTransport()
            with self.assertRaisesRegex(ProbeError, "MASTER_FIELDS"):
                collect_pages(api_key="SYNTHETIC_KEY",transport=transport,utc_clock=clock.utc,monotonic=clock.monotonic,sleeper=clock.sleep,staging=staging)
            raws=sorted((staging/"responses").glob("*.json")); sidecars=sorted((staging/"response_receipts").glob("*.receipt.json"))
            self.assertEqual((len(transport.calls),len(raws),len(sidecars)),(2,2,2))
            receipt=json.loads(sidecars[1].read_text(encoding="utf-8"))
            self.assertEqual((receipt["http_ordinal"],receipt["query_id"],receipt["status"]),(2,"Q02_MASTER_NORMAL",200))
            self.assertEqual(receipt["body_sha256"],sha256_bytes(raws[1].read_bytes()))
            self.assertNotIn("request_parameters",receipt)
            for path in staging.rglob("*"):
                if path.is_file(): self.assertNotIn(b"SYNTHETIC_KEY",path.read_bytes())

    def test_http_status_failure_is_raw_first_and_receipted(self) -> None:
        def forbidden(url: str, api_key: str, cap_bytes: int) -> HTTPResponse:
            return HTTPResponse(403,"application/json",_body({"message":"forbidden"}),url)
        with tempfile.TemporaryDirectory() as temp:
            staging=Path(temp); clock=VirtualClock()
            with self.assertRaisesRegex(ProbeError,"HTTP_STATUS"):
                collect_pages(api_key="SYNTHETIC_KEY",transport=forbidden,utc_clock=clock.utc,monotonic=clock.monotonic,sleeper=clock.sleep,staging=staging)
            self.assertEqual(len(tuple((staging/"responses").glob("*.json"))),1)
            receipt_path=next((staging/"response_receipts").glob("*.receipt.json"))
            self.assertEqual(json.loads(receipt_path.read_text(encoding="utf-8"))["status"],403)

    def test_receipt_disk_failure_prevents_semantic_parse(self) -> None:
        real_atomic=collector_module._atomic
        def failing_atomic(directory: Path, name: str, payload: bytes) -> Path:
            if directory.name=="response_receipts": raise OSError("simulated receipt disk failure")
            return real_atomic(directory,name,payload)
        with tempfile.TemporaryDirectory() as temp, patch.object(collector_module,"_atomic",side_effect=failing_atomic), patch.object(collector_module,"parse_page") as parser:
            clock=VirtualClock(); staging=Path(temp)
            with self.assertRaisesRegex(CollectorFailure,"EVIDENCE_WRITE"):
                collect_pages(api_key="SYNTHETIC_KEY",transport=FiveQueryTransport(),utc_clock=clock.utc,monotonic=clock.monotonic,sleeper=clock.sleep,staging=staging)
            parser.assert_not_called()
            self.assertEqual(len(tuple((staging/"responses").glob("*.json"))),1)
            self.assertEqual(len(tuple((staging/"response_receipts").glob("*.json"))),0)
    ...
    def test_pagination_is_prior_page_only_complete_and_not_a_retry(self) -> None:
        ...
        self.assertIsNone(receipts[0]["prior_pagination_key_sha256"])
        self.assertEqual(receipts[1]["prior_pagination_key_sha256"], sha256_bytes(b"NEXT_PAGE"))
        expected_params = dict(QUERY_PLANS[0].parameters)
        expected_params["pagination_key"] = "NEXT_PAGE"
        self.assertEqual(receipts[1]["request_parameters_sha256"], sha256_bytes(canonical_json_bytes(expected_params)))
    ...
    def test_pagination_loop_page_cap_and_global_cap_fail_closed(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp, patch("quant_research.alpha_models.data.jquants_v2_v4.collector.MAX_PAGES", 1):
            clock = VirtualClock()
        ...
        with tempfile.TemporaryDirectory() as temp, patch("quant_research.alpha_models.data.jquants_v2_v4.collector.GLOBAL_HTTP_CAP", 5):
            clock = VirtualClock()
    ...
    def test_strict_schema_duplicate_nonfinite_null_and_invalid_exrt(self) -> None:
        ...
        bad = _bar("2025-03-28", factor=0.5, ex_right="0")
        ...
        null_bar = _bar("2025-03-28", factor=1.0, ex_right=None, market_cap=None)
...
class JQuantsV2V4AuthorityAndLoaderTests(unittest.TestCase):
    def test_external_freeze_requires_canonical_exact_files_and_detects_drift(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            expected = _prepare_bound_repo(root)
            path = root / "experiments/exp_20260828_005/artifacts/expected_freeze_manifest.json"
    ...
    def test_source_drift_before_promotion_fails_without_final(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            parent = root / "data/raw/jquants_v2_v4/runs"
            self.assertFalse((parent / "exp_20260828_005_formal_001").exists())
            failure = json.loads((parent / ".exp_20260828_005_formal_001.control/failure.json").read_text(encoding="utf-8"))
    ...
    def test_no_clobber_race_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            expected = _prepare_bound_repo(root)
            clock = VirtualClock()
            final = root / "data/raw/jquants_v2_v4/runs/exp_20260828_005_formal_001"
            def race() -> None:
                final.mkdir()
    ...
    def _tamper_receipts(self, final: Path, mutation) -> None:  # type: ignore[no-untyped-def]
        ...
        sidecars = sorted((final / "response_receipts").glob("*.receipt.json"))
        self.assertEqual(len(sidecars), len(rows))
        for sidecar, row in zip(sidecars, rows):
            sidecar.write_bytes(json_file_bytes(row))
    ...
    def test_loader_rebuilds_valid_pagination_and_rejects_wrong_prior_key(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            def wrong(rows) -> None:  # type: ignore[no-untyped-def]
                rows[1]["prior_pagination_key_sha256"] = sha256_bytes(b"WRONG")
    ...
    def test_preexistence_and_transport_secret_failure_do_not_publish(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            data_root = root / "data/raw/jquants_v2_v4"
            for path in data_root.rglob("*"):
                if path.is_file(): self.assertNotIn(secret.encode("utf-8"), path.read_bytes())
            self.assertFalse((data_root / "runs/exp_20260828_005_formal_001").exists())
    ...
    def test_formal_code_filtered_master_presence_is_unconditionally_unknown(self) -> None:
        ...
        loader_source = (REPO_ROOT / "src/quant_research/alpha_models/data/jquants_v2_v4/loader.py").read_text(encoding="utf-8")
...
class JQuantsV2V4AdapterTests(unittest.TestCase):
    @classmethod
    ...
    def _registries(self) -> tuple[StableLabelRegistry, StableLabelRegistry]:
        ...
        return (
            StableLabelRegistry.from_labels("SCC", self.symbols, authority_id="JQUANTS_V2_V4_SYNTHETIC", training_partition_id="TRAIN", known_at_ms=known),
            StableLabelRegistry.from_labels("SSC", ("10", "20"), authority_id="JQUANTS_V2_V4_SYNTHETIC", training_partition_id="TRAIN", known_at_ms=known),
        )
...
class JQuantsV2V4LauncherTests(unittest.TestCase):
    ...
    def test_drifted_import_side_effect_never_executes_before_authority(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            launcher, launcher_sha, freeze_sha = _prepare_launcher_sandbox(root, b"JQUANTS_API_KEY=SYNTHETIC\n")
            collector = root / "src/quant_research/alpha_models/data/jquants_v2_v4/collector.py"
    ...
    def test_manifest_noncanonical_traversal_and_reparse_are_rejected_before_env(self) -> None:
        for mode in ("noncanonical", "traversal", "reparse"):
            with self.subTest(mode=mode), tempfile.TemporaryDirectory() as temp:
                root = Path(temp)
                launcher, launcher_sha, freeze_sha = _prepare_launcher_sandbox(root, None)
                manifest_path = root / "experiments/exp_20260828_005/artifacts/expected_freeze_manifest.json"
