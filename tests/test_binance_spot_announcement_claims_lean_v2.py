...
import binance_spot_announcement_claims_lean_v2_loader as loader
import binance_spot_announcement_claims_lean_v2_runner as runner
...
def patched_runner_layout(
    parent: pathlib.Path, *, shared_parents_exist: bool = False,
):
    repo = parent / "repo"
    processed = repo / "data" / "processed"
    version = processed / "binance_spot_announcement_claims_v2"
    runs = version / "runs"
    processed.mkdir(parents=True)
    if shared_parents_exist:
    ...
    layout = {
        ...
        "RUNS_ROOT": runs,
        "FINAL_ROOT": runs / "exp_20260826_007_formal_001",
        "STAGING_ROOT": runs / ".exp_20260826_007_formal_001.staging",
        "CONTROL_ROOT": runs / ".exp_20260826_007_formal_001.control",
    }
    with mock.patch.multiple(runner, **layout):
        yield layout
...
class FrozenCorpusTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.parameters = json.loads(
            (
                REPO_ROOT
                / "experiments/exp_20260826_007/parameters.json"
...
class LoaderAndRunnerTests(unittest.TestCase):
    ...
    def test_parent_bootstrap_missing_then_existing_is_idempotent(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            with patched_runner_layout(pathlib.Path(directory)) as layout:
                self.assertFalse(layout["VERSION_ROOT"].exists())
                runner._bootstrap_shared_parents()
                self.assertTrue(layout["RUNS_ROOT"].is_dir())
                runner._bootstrap_shared_parents()
                self.assertTrue(layout["RUNS_ROOT"].is_dir())
                self.assertFalse(layout["CONTROL_ROOT"].exists())

    def test_parent_bootstrap_rejects_file_component(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            with patched_runner_layout(pathlib.Path(directory)) as layout:
                layout["VERSION_ROOT"].write_text("file", encoding="utf-8")
                with self.assertRaises(runner.RunnerError) as captured:
                    runner._bootstrap_shared_parents()
                self.assertEqual(captured.exception.failure_code, "INFRASTRUCTURE")
                self.assertFalse(layout["CONTROL_ROOT"].exists())

    def test_parent_bootstrap_rejects_reparse_attribute(self) -> None:
        ...
        with mock.patch.object(pathlib.Path, "lstat", return_value=metadata):
            with self.assertRaises(runner.RunnerError) as captured:
                runner._ensure_existing_directory(pathlib.Path("synthetic"))
        self.assertEqual(captured.exception.failure_code, "INFRASTRUCTURE")

    def test_parent_bootstrap_rejects_escape_and_cross_drive(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            parent = pathlib.Path(directory)
            with patched_runner_layout(parent) as layout:
                with mock.patch.object(
                    runner, "VERSION_ROOT", parent / "escaped-version",
                ):
                    with self.assertRaises(runner.RunnerError):
                        runner._validate_parent_layout(
                            shared_parents_must_exist=False,
                        )
                real_drive = runner._drive

                def split_drive(path: pathlib.Path) -> str:
                    if runner._same_path(path, layout["FINAL_ROOT"]):
                        return "z:"
                    return real_drive(path)

                with mock.patch.object(runner, "_drive", side_effect=split_drive):
                    with self.assertRaises(runner.RunnerError) as captured:
                        runner._validate_parent_layout(
                            shared_parents_must_exist=False,
                        )
                self.assertEqual(captured.exception.exit_code, 24)

    def test_control_reservation_race_is_infrastructure_without_failure(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            parent = pathlib.Path(directory)
            with patched_runner_layout(
                parent, shared_parents_exist=True,
            ) as layout:
                original_mkdir = pathlib.Path.mkdir

                def racing_mkdir(path: pathlib.Path, *args, **kwargs) -> None:
                    if runner._same_path(path, layout["CONTROL_ROOT"]):
                        raise FileExistsError("synthetic reservation race")
                    original_mkdir(path, *args, **kwargs)

                with (
                    mock.patch.object(runner, "verify_bindings"),
                    mock.patch.object(
                        pathlib.Path, "mkdir", new=racing_mkdir,
                    ),
                ):
                    self.assertEqual(runner.main(self._argv()), 24)
                self.assertFalse(layout["CONTROL_ROOT"].exists())

    def test_lease_commit_failure_retains_control_without_failure(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            with patched_runner_layout(
                pathlib.Path(directory), shared_parents_exist=True,
            ) as layout:
                with (
                    mock.patch.object(runner, "verify_bindings"),
                    mock.patch.object(
                        runner, "_write_once",
                        side_effect=OSError("synthetic lease crash"),
                    ),
                ):
                    self.assertEqual(runner.main(self._argv()), 24)
                control = layout["CONTROL_ROOT"]
                self.assertTrue(control.is_dir())
                self.assertFalse((control / "lease.json").exists())
                self.assertFalse((control / "failure.json").exists())

    def test_exp006_frozen_sources_and_run_remain_untouched(self) -> None:
        expected = {
            "src/quant_research/binance_spot_announcement_claims_lean_v1_runner.py":
                "8f068d89e0fa66840e12706aa8954ca8e5585d271b2a76893abb385f182abe0b",
            "src/quant_research/binance_spot_announcement_claims_lean_v1.py":
                "552aba2e02db3c0d1ad4e500bac7f3036d24d150abd74dd4ac7e756803f15dac",
            "src/quant_research/binance_spot_announcement_claims_lean_v1_loader.py":
                "76cc371b6f10e2f87b92d7fcc71e648c245fffe8a3f42b83d6be8cc4b7dad347",
        }
        ...
        old_runs = (
            REPO_ROOT / "data/processed/binance_spot_announcement_claims_v1/runs"
        )
        self.assertFalse((old_runs / "exp_20260826_006_formal_001").exists())
        self.assertFalse(
            (old_runs / ".exp_20260826_006_formal_001.staging").exists()
        )
        self.assertFalse(
            (old_runs / ".exp_20260826_006_formal_001.control").exists()
        )

    def test_v2_semantic_sources_equal_v1_after_identity_normalization(self) -> None:
        pairs = (
            (
                "src/quant_research/binance_spot_announcement_claims_lean_v1.py",
                "src/quant_research/binance_spot_announcement_claims_lean_v2.py",
            ),
            (
                "src/quant_research/binance_spot_announcement_claims_lean_v1_loader.py",
                "src/quant_research/binance_spot_announcement_claims_lean_v2_loader.py",
            ),
        )
        for old_relative, new_relative in pairs:
            old = (REPO_ROOT / old_relative).read_text(encoding="utf-8")
            new = (REPO_ROOT / new_relative).read_text(encoding="utf-8")
            normalized = (
                new.replace("exp_20260826_007", "exp_20260826_006")
                .replace(
                    "binance_spot_announcement_claims_lean_v2",
                    "binance_spot_announcement_claims_lean_v1",
                )
                .replace(
                    "binance_spot_announcement_claims_v2",
                    "binance_spot_announcement_claims_v1",
                )
            )
            self.assertEqual(
                normalized.rstrip("\r\n"), old.rstrip("\r\n"),
    ...
    def test_runner_preexistence_is_zero_write(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patched_runner_layout(
                ...
                final.mkdir()
                before = sorted(item.name for item in layout["RUNS_ROOT"].iterdir())
                self.assertEqual(runner.main(self._argv()), 10)
                after = sorted(item.name for item in layout["RUNS_ROOT"].iterdir())
    ...
    def test_formal_paths_absent_and_no_runtime_forensics(self) -> None:
        manifest = json.loads(
            (
                REPO_ROOT
                / "experiments/exp_20260826_007/manifest.json"
            ).read_text(encoding="utf-8")
        )
        if manifest["formal_executed"] is True:
            self.assertEqual(manifest["phase"], "POSTFLIGHT_AUDITED")
            self.assertTrue(runner.FINAL_ROOT.is_dir())
            self.assertFalse(runner.STAGING_ROOT.exists())
            self.assertTrue(runner.CONTROL_ROOT.is_dir())
            self.assertTrue((runner.CONTROL_ROOT / "lease.json").is_file())
            self.assertTrue(
                (runner.CONTROL_ROOT / "authorization.json").is_file()
            )
            self.assertFalse(
                (runner.CONTROL_ROOT / "failure.json").exists()
            )
        else:
