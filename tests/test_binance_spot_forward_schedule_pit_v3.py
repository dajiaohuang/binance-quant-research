...
import binance_spot_forward_schedule_pit_v3 as collector
import binance_spot_forward_schedule_pit_v3_loader as loader
import binance_spot_forward_schedule_pit_v2 as exp009_collector


SECRET = "SENTINEL_EXP010_READ_ONLY_KEY_9f6c"
...
class WrapperHarness:
    def __init__(self, case: unittest.TestCase, *, child_exit: int = 0, include_python: bool = True) -> None:
        ...
        self.wrapper = self.repo / "src/quant_research/binance_spot_forward_schedule_pit_v3_wrapper.ps1"
        self.wrapper.parent.mkdir(parents=True)
        self.wrapper.write_bytes((ROOT / collector.BINDING_SPECS[0][2]).read_bytes())
        self.control = self.repo / "experiments/exp_20260826_010/formal_control"
        ...
        if include_python:
            ...
            fake = self.repo / "src/quant_research/binance_spot_forward_schedule_pit_v3.py"
            fake.write_text(
                "import pathlib\n"
...
class ForwardSchedulePitTests(unittest.TestCase):
    ...
    def test_wrapper_static_cleanup_and_exit_order(self) -> None:
        ...
        self.assertLess(text.index("Get-Clipboard -Raw"), text.index("Set-Clipboard -Value $clipboardOverwriteMarker"))
        self.assertLess(text.index("Set-Clipboard -Value $clipboardOverwriteMarker"), text.index("$env:BINANCE_READ_ONLY_API_KEY"))
        self.assertIn("Remove-Item Env:BINANCE_READ_ONLY_API_KEY", text)
        self.assertEqual(text.count("$clipboardOverwriteMarker='CLEARED_BY_CODEX'"), 1)
        self.assertEqual(text.count("Set-Clipboard -Value $clipboardOverwriteMarker"), 2)
        self.assertNotIn("Set-Clipboard -Value ''", text)
        self.assertEqual(text.count("Get-Clipboard -Raw"), 1)
        self.assertNotIn("Get-Clipboard", text[text.index("Set-Clipboard -Value $clipboardOverwriteMarker"):])
        self.assertEqual(text.count("$clipboardOverwriteMarker"), 3)
    ...
    def test_wrapper_valid_clipboard_source_failure_clears_parent_state(self) -> None:
        ...
        self.assertEqual(
            ...
            b'{"experiment_id":"exp_20260826_010","run_id":"exp_20260826_010_formal_001"}\n',
        )
        self.assertNotIn(SECRET, result.stdout + result.stderr)
        self.assertNotIn(SECRET.encode(), harness.reservation.read_bytes() + harness.ledger.read_bytes())

    def test_wrapper_writes_exact_nonempty_marker_twice_without_readback(self) -> None:
        harness = WrapperHarness(self, child_exit=0)
        result = harness.run(
            f"[IO.File]::AppendAllText('{harness.get_marker.as_posix()}','G'); '{SECRET}'",
            f"[IO.File]::AppendAllText('{harness.set_marker.as_posix()}', $Value + [Environment]::NewLine)",
        )
        self.assertEqual((result.returncode, result.stdout, result.stderr), (0, "", ""))
        self.assertEqual(harness.get_marker.read_text(encoding="utf-8"), "G")
        self.assertEqual(
            harness.set_marker.read_text(encoding="utf-8").splitlines(),
            ["CLEARED_BY_CODEX", "CLEARED_BY_CODEX"],
        )
        ...
        self.assertNotIn(b"CLEARED_BY_CODEX", evidence)
    ...
    def test_exp009_to_exp010_data_semantics_are_value_equal(self) -> None:
        self.assertEqual(
    ...
    def test_exp009_to_exp010_sources_differ_only_by_identity(self) -> None:
        old_collector = (SRC / "binance_spot_forward_schedule_pit_v2.py").read_text(encoding="utf-8")
        new_collector = (SRC / "binance_spot_forward_schedule_pit_v3.py").read_text(encoding="utf-8")
        normalized_collector = (
            new_collector
            .replace("exp_20260826_010", "exp_20260826_009")
            .replace("binance_spot_forward_schedule_pit_v3", "binance_spot_forward_schedule_pit_v2")
        )
        ...
        new_loader = (SRC / "binance_spot_forward_schedule_pit_v3_loader.py").read_text(encoding="utf-8")
        normalized_loader = (
            new_loader
            .replace("exp_20260826_010", "exp_20260826_009")
            .replace("binance_spot_forward_schedule_pit_v3", "binance_spot_forward_schedule_pit_v2")
    ...
    def test_exp009_to_exp010_wrapper_diff_is_only_marker_variable(self) -> None:
        old_wrapper = (SRC / "binance_spot_forward_schedule_pit_v2_wrapper.ps1").read_text(encoding="utf-8")
        new_wrapper = (SRC / "binance_spot_forward_schedule_pit_v3_wrapper.ps1").read_text(encoding="utf-8")
        normalized = (
            new_wrapper
            .replace("exp_20260826_010", "exp_20260826_009")
            .replace("binance_spot_forward_schedule_pit_v3", "binance_spot_forward_schedule_pit_v2")
            .replace("$clipboardOverwriteMarker='CLEARED_BY_CODEX'\n", "")
            .replace("$clipboardOverwriteMarker", "''")
        )
        self.assertEqual(normalized, old_wrapper)
    ...
    def test_real_workspace_formal_paths_absent_and_network_zero(self) -> None:
        ...
        wrapper_control = ROOT / "experiments/exp_20260826_010/formal_control"
