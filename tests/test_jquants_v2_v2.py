...
from quant_research.alpha_models.data.jquants_v2_v2 import adapters

from quant_research.alpha_models.data.jquants_v2_v2.collector import (
    CollectorFailure,
    HTTPResponse,
...
from quant_research.alpha_models.data.jquants_v2_v2.contracts import (
    QUERY_PLANS,
    DailyBar,
    CalendarDay,
    MasterRow,
...
from quant_research.alpha_models.data.jquants_v2_v2.loader import (
    EXPECTED_9433_DATES,
    causal_prices,
    derive_adjacent_presence,
    merge_and_validate,
    parse_page,
    trusted_rebuild,
)
...
def _prepare_bound_repo(root: Path) -> str:
    entries = []
    ...
    manifest = {"files": sorted(entries, key=lambda row: row["path"].encode("utf-8")), "schema_version": "JQUANTS_V2_V2_EXTERNAL_FREEZE_V1"}
    target = root / "experiments/exp_20260828_003/artifacts/expected_freeze_manifest.json"
...
def _successful_temp_run(root: Path, transport: FiveQueryTransport | None = None) -> tuple[Path, Path, str]:
    ...
    parent = root / "data/raw/jquants_v2_v2/runs"
    return parent / "exp_20260828_003_formal_001", parent / ".exp_20260828_003_formal_001.control", expected


def _prepare_launcher_sandbox(root: Path, env_bytes: bytes | None) -> tuple[Path, str, str]:
    launcher = root / "src/quant_research/alpha_models/data/jquants_v2_v2/launcher.ps1"
    launcher.parent.mkdir(parents=True, exist_ok=True)
    shutil.copyfile(REPO_ROOT / FREEZE_FILES[5], launcher)
    freeze = root / "experiments/exp_20260828_003/artifacts/expected_freeze_manifest.json"
    ...
    package = root / "quant_research/alpha_models/data/jquants_v2_v2"
    package.mkdir(parents=True, exist_ok=True)
    for directory in (root / "quant_research", root / "quant_research/alpha_models", root / "quant_research/alpha_models/data", package):
        (directory / "__init__.py").write_text("", encoding="utf-8")
    (package / "collector.py").write_text(
        "import json,os,sys\n"
        "from pathlib import Path\n"
        "Path('child_marker.json').write_text(json.dumps({'argv':sys.argv[1:],'key_present':bool(os.environ.get('JQUANTS_API_KEY'))}),encoding='utf-8')\n"
        "raise SystemExit(int(os.environ.get('FAKE_CHILD_EXIT','0')))\n",
        encoding="utf-8",
    )
    return launcher, sha256_bytes(launcher.read_bytes()), sha256_bytes(freeze.read_bytes())
...
class JQuantsV2V2QueryContractTests(unittest.TestCase):
    ...
    def test_pagination_loop_page_cap_and_global_cap_fail_closed(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp, patch("quant_research.alpha_models.data.jquants_v2_v2.collector.MAX_PAGES", 1):
            clock = VirtualClock()
        ...
        with tempfile.TemporaryDirectory() as temp, patch("quant_research.alpha_models.data.jquants_v2_v2.collector.GLOBAL_HTTP_CAP", 5):
            clock = VirtualClock()
...
class JQuantsV2V2AuthorityAndLoaderTests(unittest.TestCase):
    def test_external_freeze_requires_canonical_exact_files_and_detects_drift(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            expected = _prepare_bound_repo(root)
            path = root / "experiments/exp_20260828_003/artifacts/expected_freeze_manifest.json"
    ...
    def test_source_drift_before_promotion_fails_without_final(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            parent = root / "data/raw/jquants_v2_v2/runs"
            self.assertFalse((parent / "exp_20260828_003_formal_001").exists())
            failure = json.loads((parent / ".exp_20260828_003_formal_001.control/failure.json").read_text(encoding="utf-8"))
    ...
    def test_no_clobber_race_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            expected = _prepare_bound_repo(root)
            clock = VirtualClock()
            final = root / "data/raw/jquants_v2_v2/runs/exp_20260828_003_formal_001"
            def race() -> None:
                final.mkdir()
    ...
    def test_preexistence_and_transport_secret_failure_do_not_publish(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            data_root = root / "data/raw/jquants_v2_v2"
            for path in data_root.rglob("*"):
                if path.is_file(): self.assertNotIn(secret.encode("utf-8"), path.read_bytes())
            self.assertFalse((data_root / "runs/exp_20260828_003_formal_001").exists())

    def test_single_or_filtered_master_snapshot_never_claims_presence(self) -> None:
        pages = _five_pages()
        loaded = merge_and_validate(pages)
        calendar = loaded.calendar
        one = derive_adjacent_presence(loaded.masters, calendar, full_snapshot_dates=())
        self.assertEqual((one.status, one.reason), ("UNKNOWN", "MASTER_QUERY_NOT_FULL_SNAPSHOT"))
        first = pages[1].records[0]
        assert isinstance(first, MasterRow)
        second = replace(first, snapshot_date="2025-03-28", policy_observation_ms=policy_time_ms("2025-03-28", 8, 0), available_at_ms=max(policy_time_ms("2025-03-28", 8, 0), first.received_at_ms))
        full = derive_adjacent_presence((second, first), calendar, full_snapshot_dates=("2025-03-28", "2025-03-31"))
        self.assertEqual(full.status, "DERIVED_PRESENCE")
        april_policy = policy_time_ms("2025-04-02", 8, 0)
        nonadjacent = derive_adjacent_presence((second, replace(first, snapshot_date="2025-04-02", policy_observation_ms=april_policy, available_at_ms=max(april_policy, first.received_at_ms))), calendar, full_snapshot_dates=("2025-03-28", "2025-04-02"))
        self.assertEqual(nonadjacent.status, "UNKNOWN")
...
class JQuantsV2V2AdapterTests(unittest.TestCase):
    @classmethod
...
class JQuantsV2V2LauncherTests(unittest.TestCase):
    ...
    def test_launcher_rejects_preexisting_env_bad_grammar_cap_and_directory(self) -> None:
        cases = (
            (b"JQUANTS_API_KEY=A\nEXTRA=1\n", None, 43),
            (b"JQUANTS_API_KEY=" + b"A" * 4097, None, 42),
