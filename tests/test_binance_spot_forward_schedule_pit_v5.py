...
import binance_spot_forward_schedule_pit_v5 as collector
import binance_spot_forward_schedule_pit_v5_loader as loader
import binance_spot_forward_schedule_pit_v4 as exp001_collector


SECRET = "SENTINELEXP002_READ-ONLY_KEY9F6C"
...
class WrapperHarness:
    def __init__(self, case: unittest.TestCase, *, child_exit: int = 0, include_python: bool = True) -> None:
        ...
        self.wrapper = self.repo / "src/quant_research/binance_spot_forward_schedule_pit_v5_wrapper.ps1"
        self.wrapper.parent.mkdir(parents=True)
        self.wrapper.write_bytes((ROOT / collector.BINDING_SPECS[0][2]).read_bytes())
        self.control = self.repo / "experiments/exp_20260827_002/formal_control"
        ...
        if include_python:
            ...
            fake = self.repo / "src/quant_research/binance_spot_forward_schedule_pit_v5.py"
            fake.write_text(
                "import os, pathlib\n"
...
class ForwardSchedulePitTests(unittest.TestCase):
    ...
    def test_exp001_to_exp002_data_semantics_are_value_equal(self) -> None:
        self.assertEqual(
            [(x.ordinal, x.endpoint_id, x.url, x.api_key_header, x.body_cap) for x in collector.ENDPOINTS],
            [(x.ordinal, x.endpoint_id, x.url, x.api_key_header, x.body_cap) for x in exp001_collector.ENDPOINTS],
        )
        self.assertEqual(
            (collector.MAX_TOTAL_BODY_BYTES, collector.REQUEST_TIMEOUT_SECONDS, collector.TOTAL_WALL_SECONDS),
            (exp001_collector.MAX_TOTAL_BODY_BYTES, exp001_collector.REQUEST_TIMEOUT_SECONDS, exp001_collector.TOTAL_WALL_SECONDS),
        )
        bodies = dict(zip((item.endpoint_id for item in collector.ENDPOINTS), response_bodies(), strict=True))
        self.assertEqual(collector.derive_rows(bodies), exp001_collector.derive_rows(bodies))

    def test_exp001_to_exp002_sources_differ_only_by_identity(self) -> None:
        old_collector = (SRC / "binance_spot_forward_schedule_pit_v4.py").read_text(encoding="utf-8")
        new_collector = (SRC / "binance_spot_forward_schedule_pit_v5.py").read_text(encoding="utf-8")
        normalized_collector = (
            new_collector
            .replace("exp_20260827_002", "exp_20260827_001")
            .replace("binance_spot_forward_schedule_pit_v5", "binance_spot_forward_schedule_pit_v4")
        )
        self.assertEqual(normalized_collector, old_collector)

        old_loader = (SRC / "binance_spot_forward_schedule_pit_v4_loader.py").read_text(encoding="utf-8")
        new_loader = (SRC / "binance_spot_forward_schedule_pit_v5_loader.py").read_text(encoding="utf-8")
        normalized_loader = (
            new_loader
            .replace("exp_20260827_002", "exp_20260827_001")
            .replace("binance_spot_forward_schedule_pit_v5", "binance_spot_forward_schedule_pit_v4")
    ...
    def test_real_workspace_formal_paths_absent_and_network_zero(self) -> None:
        ...
        wrapper_control = ROOT / "experiments/exp_20260827_002/formal_control"
