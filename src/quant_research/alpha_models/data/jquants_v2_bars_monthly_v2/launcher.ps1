...
$runId = 'exp_20260828_008_bootstrap_formal_001'
$experimentId = 'exp_20260828_008'
$apiKey = $null
$envOwned = $false
$ledger = $null
...
try {
    ...
    $freezePath = Join-Path $repoRoot 'experiments\exp_20260828_008\artifacts\candidate_freeze_manifest.json'
...
try:
    ...
    if raw!=(json.dumps(value,ensure_ascii=False,allow_nan=False,sort_keys=True,indent=2)+'\n').encode() or type(value) is not dict or set(value)!={'files','schema_version'} or value['schema_version']!='JQUANTS_V2_BARS_MONTHLY_V2_CANDIDATE_FREEZE_V1': fail()
    seen=set()
...
'@
    ...
    $controlRoot = Join-Path $repoRoot 'experiments\exp_20260828_008\formal_control'
    [IO.Directory]::CreateDirectory($controlRoot) | Out-Null
    & $pythonPath -I -B -m quant_research.alpha_models.data.jquants_v2_bars_monthly_v2 --repo-root $repoRoot --reuse-preflight-check | Out-Null
    ...
    Add-Ledger 'FREEZE_READ_ONLY_REUSE_PREFLIGHT_AND_O_EXCL_CONTROL_RESERVATION_PASS'

    & $pythonPath -I -B -m quant_research.alpha_models.data.jquants_v2_bars_monthly_v2 --repo-root $repoRoot --reserve-and-emit-reuse | Out-Null
    if ($LASTEXITCODE -ne 0) { throw 'ATTEMPT_RESERVATION_OR_REUSE_EMIT_FAILED' }
    $attemptStaging = Join-Path $repoRoot "data\raw\jquants_v2_bars_monthly_v2\runs\.$runId.staging"
    $reusePreflightPath = Join-Path $attemptStaging 'preflight_reuse_registry.json'
    Get-StrictFile $reusePreflightPath | Out-Null
    Add-Ledger 'ATTEMPT_OWNED_REUSE_REGISTRY_EMITTED_BEFORE_ENV'
    ...
    Add-Ledger 'ENV_ONLY_KEY_READY_AFTER_REUSE_PREFLIGHT'

    Set-Location -LiteralPath $repoRoot
    & $pythonPath -I -B -m quant_research.alpha_models.data.jquants_v2_bars_monthly_v2 --repo-root $repoRoot --formal-bootstrap-pre-reserved
