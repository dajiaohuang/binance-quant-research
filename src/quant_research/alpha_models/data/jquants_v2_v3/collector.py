...
EXPECTED_FREEZE_RELATIVE = Path("experiments/exp_20260828_004/artifacts/expected_freeze_manifest.json")
...
def verify_expected_freeze(repo_root:Path,expected_sha256:str)->dict[str,object]:
    ...
    if set(manifest)!={"files","schema_version"} or manifest["schema_version"]!="JQUANTS_V2_V3_EXTERNAL_FREEZE_V1" or type(manifest["files"]) is not list: raise CollectorFailure("FREEZE_SCHEMA")
    seen=set()
...
def collect_and_publish(*,repo_root:Path,expected_freeze_sha256:str,api_key:str,transport:Transport=urllib_transport,utc_clock:Callable[[],int]=lambda:time.time_ns()//1_000_000,monotonic:Callable[[],float]=time.monotonic,sleeper:Callable[[float],None]=time.sleep,before_promotion:Callable[[],None]|None=None)->dict[str,object]:
    ...
    parent=repo_root/"data/raw/jquants_v2_v3/runs"; parent.mkdir(parents=True,exist_ok=True)
    current=repo_root
    for part in ("data","raw","jquants_v2_v3","runs"):
