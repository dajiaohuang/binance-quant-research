...
def derive_adjacent_presence(rows: Iterable[MasterRow], calendar: Iterable[CalendarDay], *, full_snapshot_dates: tuple[str,...]) -> PresenceResolution:
    master=tuple(rows); sessions=tuple(row.session_date for row in sorted(calendar,key=lambda x:x.session_date) if row.is_session)
    dates=tuple(sorted({row.snapshot_date for row in master}))
    if type(full_snapshot_dates) is not tuple or tuple(sorted(full_snapshot_dates))!=full_snapshot_dates or len(set(full_snapshot_dates))!=len(full_snapshot_dates) or any(date_text(item)!=item for item in full_snapshot_dates): raise ProbeError("FULL_SNAPSHOT_SCOPE")
    if any(day not in full_snapshot_dates for day in dates): return PresenceResolution("UNKNOWN",(),"MASTER_QUERY_NOT_FULL_SNAPSHOT")
    if len(dates)<2: return PresenceResolution("UNKNOWN",(),"FEWER_THAN_TWO_MASTER_SNAPSHOTS")
    intervals=[]
    for left,right in zip(dates,dates[1:]):
        if left not in sessions or sessions.index(left)+1>=len(sessions) or sessions[sessions.index(left)+1]!=right: return PresenceResolution("UNKNOWN",(),"SNAPSHOTS_NOT_ADJACENT_OFFICIAL_SESSIONS")
        a={row.symbol for row in master if row.snapshot_date==left}; b={row.symbol for row in master if row.snapshot_date==right}
        intervals.extend((symbol,left,right) for symbol in sorted(a&b))
    return PresenceResolution("DERIVED_PRESENCE",tuple(intervals),"ADJACENT_FULL_MASTER_INTERSECTION_ONLY")
...
def trusted_rebuild(staging: Path) -> RebuildResult:
    ...
    expected_summary={"artifact_state":"JQUANTS_V2_FREE_SOURCE_PROBE_ACQUIRED","bar_rows":len(loaded.bars),"calendar_rows":len(loaded.calendar),"empirical_authorized":False,"experiment_id":"exp_20260828_003","historical_eligibility_ready":False,"http_request_count":len(receipts),"logical_query_count":5,"master_rows":len(loaded.masters),"raw_tree_sha256":manifest["raw_tree_sha256"],"retry_count":0,"run_id":RUN_ID,"strict_eligible_count":0,"terminal_status":"NEEDS_MORE_DATA"}
