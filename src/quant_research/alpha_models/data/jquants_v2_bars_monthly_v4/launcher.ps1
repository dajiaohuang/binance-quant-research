...
$batchId = 'exp_20260828_010_monthly_formal_001'
$experimentId = 'exp_20260828_010'
$apiKey = $null
$envOwned = $false
$ledger = $null
...
try {
    ...
    $freezePath = Join-Path $repoRoot 'experiments\exp_20260828_010\artifacts\candidate_freeze_manifest.json'
...
try:
    ...
    if raw!=(json.dumps(value,ensure_ascii=False,allow_nan=False,sort_keys=True,indent=2)+'\n').encode() or type(value) is not dict or set(value)!={'files','schema_version'} or value['schema_version']!='JQUANTS_V2_BARS_MONTHLY_V4_CANDIDATE_FREEZE_V1': fail()
    seen=set()
...
'@
    ...
    # Complete read-only boundary: source/raw/registry/sessions and exp005/006
    # are revalidated before any control, batch, staging, ledger or env action.
    & $pythonPath -I -B -m quant_research.alpha_models.data.jquants_v2_bars_monthly_v4 --repo-root $repoRoot --source-preflight-check | Out-Null
    if ($LASTEXITCODE -ne 0) { throw 'SOURCE_PREFLIGHT_FAILED' }
    $preflightPassed = $true

    function Add-Ledger([string]$Event) {
        if (-not $script:preflightPassed) { throw 'WRITE_BEFORE_SOURCE_PREFLIGHT_PASS' }
    ...
    $controlRoot = Join-Path $repoRoot 'experiments\exp_20260828_010\formal_control'
    ...
    Add-Ledger 'READ_ONLY_SOURCE_PREFLIGHT_AND_O_EXCL_CONTROL_PASS'

    & $pythonPath -I -B -m quant_research.alpha_models.data.jquants_v2_bars_monthly_v4 --repo-root $repoRoot --batch-id $batchId --reserve-batch-and-emit-source-binding | Out-Null
    if ($LASTEXITCODE -ne 0) { throw 'BATCH_RESERVATION_OR_SOURCE_BINDING_FAILED' }
    Add-Ledger 'BATCH_SOURCE_BINDING_EMITTED_BEFORE_ENV'
    ...
    & $pythonPath -I -B -m quant_research.alpha_models.data.jquants_v2_bars_monthly_v4 --repo-root $repoRoot --batch-id $batchId --formal-monthly-pre-reserved
    if ($LASTEXITCODE -ne 0) { throw 'FORMAL_MONTHLY_FAILED' }
    Add-Ledger 'FORMAL_MONTHLY_EXIT_0'; exit 0
}
catch {
