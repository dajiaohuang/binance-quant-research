...
$batchId = 'exp_20260828_010_monthly_formal_003'
$experimentId = 'exp_20260828_012'
$apiKey = $null
$envOwned = $false
$ledger = $null
...
try {
    ...
    $freezePath = Join-Path $repoRoot 'experiments\exp_20260828_012\artifacts\candidate_freeze_manifest.json'
...
try:
    ...
    if raw!=(json.dumps(value,ensure_ascii=False,allow_nan=False,sort_keys=True,indent=2)+'\n').encode() or type(value) is not dict or set(value)!={'files','schema_version'} or value['schema_version']!='JQUANTS_V2_BARS_MONTHLY_V6_CANDIDATE_FREEZE_V1': fail()
    seen=set()
...
'@
    ...
    & $pythonPath -I -B -m quant_research.alpha_models.data.jquants_v2_bars_monthly_v6 --repo-root $repoRoot --recovery-preflight-check | Out-Null
    ...
    $controlRoot = Join-Path $repoRoot 'experiments\exp_20260828_012\formal_control'
    ...
    Add-Ledger 'READ_ONLY_GENERIC_RECOVERY_PREFLIGHT_AND_O_EXCL_CONTROL_PASS'

    & $pythonPath -I -B -m quant_research.alpha_models.data.jquants_v2_bars_monthly_v6 --repo-root $repoRoot --reserve-recovery-batch | Out-Null
    if ($LASTEXITCODE -ne 0) { throw 'RECOVERY_BATCH_RESERVATION_FAILED' }
    Add-Ledger 'GENERIC_POINTER_REGISTRY_EMITTED_BEFORE_ENV'
    ...
    Add-Ledger 'ENV_ONLY_KEY_READY_AFTER_GENERIC_POINTER_REGISTRY'

    Set-Location -LiteralPath $repoRoot
    & $pythonPath -I -B -m quant_research.alpha_models.data.jquants_v2_bars_monthly_v6 --repo-root $repoRoot --formal-recovery-pre-reserved
