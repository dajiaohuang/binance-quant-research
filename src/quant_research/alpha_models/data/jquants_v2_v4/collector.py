...
EXPECTED_FREEZE_RELATIVE = Path("experiments/exp_20260828_005/artifacts/expected_freeze_manifest.json")
...
def verify_expected_freeze(repo_root:Path,expected_sha256:str)->dict[str,object]:
    ...
    if set(manifest)!={"files","schema_version"} or manifest["schema_version"]!="JQUANTS_V2_V4_EXTERNAL_FREEZE_V1" or type(manifest["files"]) is not list: raise CollectorFailure("FREEZE_SCHEMA")
    seen=set()
...
def _persist_accepted_response(*,responses:Path,response_receipts:Path,plan:QueryPlan,page:int,http_ordinal:int,params:dict[str,str],prior_key:str|None,response:HTTPResponse,content_type:str,sent:int,received:int,waited:float)->tuple[dict[str,object],dict[str,object]]:
    rel=f"responses/{plan.ordinal:02d}_{plan.query_id}_page_{page:04d}.json"
    receipt_name=f"{http_ordinal:04d}_{plan.ordinal:02d}_{plan.query_id}_page_{page:04d}.receipt.json"
    digest=sha256_bytes(response.body)
    entry={"bytes":len(response.body),"http_ordinal":http_ordinal,"page_number":page,"path":rel,"query_ordinal":plan.ordinal,"sha256":digest}
    receipt={"body_bytes":len(response.body),"body_sha256":digest,"content_type":content_type,"http_ordinal":http_ordinal,"page_number":page,"path":rel,"prior_pagination_key_sha256":None if prior_key is None else sha256_bytes(prior_key.encode("utf-8")),"query_id":plan.query_id,"query_ordinal":plan.ordinal,"request_parameters_sha256":sha256_bytes(canonical_json_bytes(params)),"sent_at_ms":sent,"received_at_ms":received,"status":response.status,"redirect_count":0,"pacing_wait_ms":int(round(waited*1000))}
    try:
        _atomic(responses,Path(rel).name,response.body)
        _atomic(response_receipts,receipt_name,json_file_bytes(receipt))
    except OSError as exc:
        raise CollectorFailure("EVIDENCE_WRITE") from exc
...
def collect_pages(*,api_key:str,transport:Transport,utc_clock:Callable[[],int],monotonic:Callable[[],float],sleeper:Callable[[float],None],staging:Path)->tuple[list[object],list[dict[str,object]],list[dict[str,object]]]:
    validate_key(api_key); responses=staging/"responses"; responses.mkdir(exist_ok=False); response_receipts=staging/"response_receipts"; response_receipts.mkdir(exist_ok=False)
    ...
    for plan in QUERY_PLANS:
        ...
        while True:
            ...
            if type(response) is not HTTPResponse or type(response.status) is not int or not 100<=response.status<=599 or type(response.body) is not bytes or response.final_url!=request_url or response.redirect_count!=0 or 300<=response.status<400: raise CollectorFailure("REDIRECT")
            ...
            entry,receipt=_persist_accepted_response(responses=responses,response_receipts=response_receipts,plan=plan,page=page,http_ordinal=http_ordinal,params=params,prior_key=key,response=response,content_type=content_type,sent=sent,received=received,waited=waited)
            entries.append(entry); receipts.append(receipt)
...
def collect_and_publish(*,repo_root:Path,expected_freeze_sha256:str,api_key:str,transport:Transport=urllib_transport,utc_clock:Callable[[],int]=lambda:time.time_ns()//1_000_000,monotonic:Callable[[],float]=time.monotonic,sleeper:Callable[[float],None]=time.sleep,before_promotion:Callable[[],None]|None=None)->dict[str,object]:
    ...
    parent=repo_root/"data/raw/jquants_v2_v4/runs"; parent.mkdir(parents=True,exist_ok=True)
    current=repo_root
    for part in ("data","raw","jquants_v2_v4","runs"):
