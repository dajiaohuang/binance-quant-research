...
def _nullable_text(value: object, code: str) -> str | None:
    return None if value is None else text(value, code)
...
def _master(value: object, raw_sha: str, received: int) -> MasterRow:
    ...
    values=[text(row[key],key.upper()) for key in ("CoName","CoNameEn","S17","S17Nm","S33","S33Nm","ScaleCat","Mkt","MktNm","Mrgn","MrgnNm","ProdCat")]
...
def _bar(value: object, raw_sha: str, received: int) -> DailyBar:
    ...
    return DailyBar(day,raw_code,symbol_from_code(raw_code),_nullable(row["O"],"O"),_nullable(row["H"],"H"),_nullable(row["L"],"L"),_nullable(row["C"],"C"),text(row["UL"],"UL"),text(row["LL"],"LL"),_nullable(row["Vo"],"VO"),_nullable(row["Va"],"VA"),finite(row["AdjFactor"],"FACTOR",positive=True),_nullable(row["AdjO"],"ADJO"),_nullable(row["AdjH"],"ADJH"),_nullable(row["AdjL"],"ADJL"),_nullable(row["AdjC"],"ADJC"),_nullable(row["AdjVo"],"ADJVO"),_nullable(row["MktCap"],"MARKET_CAP"),_nullable_text(row["ExRT"],"EXRT"),policy,received,max(policy,received),raw_sha)
...
def parse_page(plan: QueryPlan, *, page_number: int, status: int, body: bytes, received_at_ms: int) -> ParsedPage:
    ...
    for row in records:
        ...
        if plan.ordinal==2 and (row.snapshot_date!="2025-03-31" or row.symbol!="9433" or row.product_category!="011"): raise ProbeError("MASTER_NORMAL")  # type: ignore[union-attr]
        if plan.ordinal==3 and (row.snapshot_date!="2025-03-31" or row.symbol!="9433" or row.product_category!="011"): raise ProbeError("MASTER_NONTRADING_MAPPING")  # type: ignore[union-attr]
...
def merge_and_validate(pages: Iterable[ParsedPage]) -> LoadedProbe:
    ...
    if tuple(row.holiday_division for row in calendar)!=("1","0","0","1"): raise ProbeError("CALENDAR_HOLDIV")
    if len(normal_master)!=1 or len(mapped_master)!=1: raise ProbeError("MASTER_EXACT_ONE")
    master_fields=("snapshot_date","raw_code","symbol","company_name","company_name_en","sector17_code","sector17_name","sector33_code","sector33_name","scale_category","market_code","market_name","margin_code","margin_name","product_category")
    ...
    bar_fields=("session_date","raw_code","symbol","o","h","low","c","upper_limit","lower_limit","volume","amount","adjustment_factor","adjusted_o","adjusted_h","adjusted_low","adjusted_c","adjusted_volume","market_cap","ex_right_type")
    ...
    if len(split)!=1 or split[0].adjustment_factor!=0.5 or split[0].ex_right_type!="1": raise ProbeError("SPLIT_EXPECTATION")
...
def trusted_rebuild(staging: Path) -> RebuildResult:
    ...
    expected_top={"responses","response_receipts","query_plan.json","receipts.jsonl","acquisition_manifest.json","summary.json"}
    ...
    receipt_paths=tuple((staging/"response_receipts").iterdir())
    if any(not p.is_file() or p.is_symlink() for p in receipt_paths) or len(receipt_paths)!=len(receipts): raise ProbeError("RECEIPT_FILE_TYPE")
    pages=[]; previous_query=0; previous_page=0; previous_key=None; previous_sent=None
    expected_receipt_keys={"body_bytes","body_sha256","content_type","http_ordinal","page_number","path","prior_pagination_key_sha256","query_id","query_ordinal","request_parameters_sha256","sent_at_ms","received_at_ms","status","redirect_count","pacing_wait_ms"}
    for ordinal,receipt in enumerate(receipts,1):
        ...
        if page>1:
            ...
            expected_prior_hash=sha256_bytes(previous_key.encode("utf-8"))
        if receipt["prior_pagination_key_sha256"]!=expected_prior_hash or receipt["request_parameters_sha256"]!=sha256_bytes(canonical_json_bytes(expected)): raise ProbeError("PAGINATION_CHAIN")
        ...
        receipt_name=f"{ordinal:04d}_{q:02d}_{QUERY_PLANS[q-1].query_id}_page_{page:04d}.receipt.json"
        receipt_path=staging/"response_receipts"/receipt_name
        if not receipt_path.is_file() or receipt_path.is_symlink(): raise ProbeError("RECEIPT_FILE_TYPE")
        receipt_raw=receipt_path.read_bytes(); receipt_doc=strict_json(receipt_raw)
        if receipt_raw!=json_file_bytes(receipt_doc) or receipt_doc!=receipt: raise ProbeError("RECEIPT_SIDECAR")
    ...
    expected_summary={"artifact_state":"JQUANTS_V2_FREE_SOURCE_PROBE_ACQUIRED","bar_rows":len(loaded.bars),"calendar_rows":len(loaded.calendar),"empirical_authorized":False,"experiment_id":"exp_20260828_005","historical_eligibility_ready":False,"http_request_count":len(receipts),"logical_query_count":5,"master_rows":len(loaded.masters),"raw_tree_sha256":manifest["raw_tree_sha256"],"retry_count":0,"run_id":RUN_ID,"strict_eligible_count":0,"terminal_status":"NEEDS_MORE_DATA"}
