...
$controlParent = Join-Path $repoRoot 'experiments\exp_20260828_005\formal_control'
$reservationPath = Join-Path $controlParent 'exp_20260828_005_formal_001.reservation.lock'
$ledgerPath = Join-Path $controlParent 'exp_20260828_005_formal_001.stage_ledger.jsonl'
...
try {
    ...
    try {
        $reservationBytes = (New-Object Text.UTF8Encoding($false)).GetBytes('{"experiment_id":"exp_20260828_005","run_id":"exp_20260828_005_formal_001"}' + "`n")
...
try:
    ...
    if raw!=canonical or type(value) is not dict or set(value)!={'files','schema_version'} or value['schema_version']!='JQUANTS_V2_V4_EXTERNAL_FREEZE_V1' or type(value['files']) is not list: fail()
    required={
      'experiments/exp_20260828_005/artifacts/schema.json','experiments/exp_20260828_005/artifacts/source_contract.json','experiments/exp_20260828_005/parameters.json',
      'src/quant_research/alpha_models/data/jquants_v2_v4/__init__.py','src/quant_research/alpha_models/data/jquants_v2_v4/adapters.py','src/quant_research/alpha_models/data/jquants_v2_v4/collector.py','src/quant_research/alpha_models/data/jquants_v2_v4/contracts.py','src/quant_research/alpha_models/data/jquants_v2_v4/launcher.ps1','src/quant_research/alpha_models/data/jquants_v2_v4/loader.py','tests/test_jquants_v2_v4.py'}
    seen=set()
...
root=os.path.abspath(sys.argv[1]); base=os.path.join(root,'src','quant_research','alpha_models','data','jquants_v2_v4')
packages=(('quant_research',os.path.join(root,'src','quant_research')),('quant_research.alpha_models',os.path.join(root,'src','quant_research','alpha_models')),('quant_research.alpha_models.data',os.path.join(root,'src','quant_research','alpha_models','data')),('quant_research.alpha_models.data.jquants_v2_v4',base))
...
for leaf in ('contracts','loader','collector'):
    name='quant_research.alpha_models.data.jquants_v2_v4.'+leaf
...
raise SystemExit(sys.modules['quant_research.alpha_models.data.jquants_v2_v4.collector'].main(['--execute','--expected-freeze-manifest-sha256',sys.argv[2]]))
'@
...
try {
    ...
    $freezePath = Join-Path $repoRoot 'experiments\exp_20260828_005\artifacts\expected_freeze_manifest.json'
