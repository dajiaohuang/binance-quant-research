...
EXPERIMENT_ID = "exp_20260828_005"
RUN_ID = "exp_20260828_005_formal_001"
VERSION = "JQUANTS_V2_FREE_SOURCE_PROBE_OFFLINE_CONTRACT_V4"
MAX_PAGES = 25
GLOBAL_HTTP_CAP = 60
...
# Japan Standard Time has no daylight-saving transition.  A fixed offset keeps
# the frozen launcher child source-only under ``python -I -S`` on Windows,
# where the optional ``tzdata`` package is intentionally unavailable.
JST = timezone(timedelta(hours=9), name="JST")
CALENDAR_FIELDS = frozenset(("Date", "HolDiv"))
MASTER_FIELDS = frozenset(("Date", "Code", "CoName", "CoNameEn", "S17", "S17Nm", "S33", "S33Nm", "ScaleCat", "Mkt", "MktNm", "Mrgn", "MrgnNm", "ProdCat"))
BAR_FIELDS = frozenset(("Date", "Code", "O", "H", "L", "C", "UL", "LL", "Vo", "Va", "AdjFactor", "AdjO", "AdjH", "AdjL", "AdjC", "AdjVo", "MktCap", "ExRT"))
...
class MasterRow:
    ...
    def __post_init__(self) -> None:
        ...
        for name in ("company_name","company_name_en","sector17_code","sector17_name","sector33_code","sector33_name","scale_category","market_code","market_name","margin_code","margin_name","product_category"):
...
class DailyBar:
    ...
    def __post_init__(self) -> None:
        ...
        if self.upper_limit not in ("0", "1") or self.lower_limit not in ("0", "1"): raise ProbeError("LIMIT_FLAG")
        if self.ex_right_type not in (None, "1", "2", "3"): raise ProbeError("EXRT")
        if self.market_cap is not None and finite(self.market_cap, "MARKET_CAP") < 0: raise ProbeError("MARKET_CAP")
        ...
        if all(item is None for item in raw):
            if any(item is not None for item in adj) or self.volume not in (None,0,0.0) or self.amount not in (None,0,0.0) or self.adjusted_volume not in (None,0,0.0) or self.market_cap is not None: raise ProbeError("NULL_BAR")
