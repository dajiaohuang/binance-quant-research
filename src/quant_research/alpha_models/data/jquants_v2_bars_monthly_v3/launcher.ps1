...
$runId = 'exp_20260828_009_bootstrap_formal_001'
$experimentId = 'exp_20260828_009'
$apiKey = $null
$envOwned = $false
$ledger = $null
$reusePreflightPassed = $false
...
try {
    ...
    $freezePath = Join-Path $repoRoot 'experiments\exp_20260828_009\artifacts\candidate_freeze_manifest.json'
...
try:
    ...
    if raw!=(json.dumps(value,ensure_ascii=False,allow_nan=False,sort_keys=True,indent=2)+'\n').encode() or type(value) is not dict or set(value)!={'files','schema_version'} or value['schema_version']!='JQUANTS_V2_BARS_MONTHLY_V3_CANDIDATE_FREEZE_V1': fail()
    seen=set()
...
'@
    ...
    # This is the complete preflight boundary. Everything above is read-only.
    # -B prevents bytecode cache creation. No control/staging/ledger/temp/registry
    # path exists or is created by this invocation.
    & $pythonPath -I -B -m quant_research.alpha_models.data.jquants_v2_bars_monthly_v3 --repo-root $repoRoot --reuse-preflight-check | Out-Null
    if ($LASTEXITCODE -ne 0) { throw 'REUSE_PREFLIGHT_FAILED' }
    $reusePreflightPassed = $true

    function Add-Ledger([string]$Event) {
        if (-not $script:reusePreflightPassed) { throw 'WRITE_BEFORE_REUSE_PREFLIGHT_PASS' }
    ...
    $controlRoot = Join-Path $repoRoot 'experiments\exp_20260828_009\formal_control'
    ...
    try {
        $body = (New-Object Text.UTF8Encoding($false)).GetBytes('{"experiment_id":"' + $experimentId + '","run_id":"' + $runId + '"}' + "`n")
    ...
    Add-Ledger 'ZERO_WRITE_REUSE_PREFLIGHT_AND_O_EXCL_CONTROL_RESERVATION_PASS'

    & $pythonPath -I -B -m quant_research.alpha_models.data.jquants_v2_bars_monthly_v3 --repo-root $repoRoot --reserve-and-emit-source-binding | Out-Null
    if ($LASTEXITCODE -ne 0) { throw 'ATTEMPT_RESERVATION_OR_SOURCE_BINDING_EMIT_FAILED' }
    $attemptStaging = Join-Path $repoRoot "data\raw\jquants_v2_bars_monthly_v3\runs\.$runId.staging"
    Get-StrictFile (Join-Path $attemptStaging 'preflight_source_binding.json') | Out-Null
    Add-Ledger 'ATTEMPT_OWNED_SOURCE_BINDING_EMITTED_BEFORE_ENV'
    ...
    Add-Ledger 'ENV_ONLY_KEY_READY_AFTER_SOURCE_BINDING'

    Set-Location -LiteralPath $repoRoot
    & $pythonPath -I -B -m quant_research.alpha_models.data.jquants_v2_bars_monthly_v3 --repo-root $repoRoot --formal-bootstrap-pre-reserved
    if ($LASTEXITCODE -ne 0) { throw 'FORMAL_BOOTSTRAP_FAILED' }
    Add-Ledger 'FORMAL_BOOTSTRAP_EXIT_0'; exit 0
}
catch {
