...
$experimentId = 'exp_20260827_002'
$runId = 'exp_20260827_002_formal_001'
...
$controlParent = [System.IO.Path]::GetFullPath(
    (Join-Path $repoRoot 'experiments\exp_20260827_002\formal_control')
)
...
try {
    $reservationBytes = $utf8NoBom.GetBytes(
        '{"experiment_id":"exp_20260827_002","run_id":"exp_20260827_002_formal_001"}' + "`n"
    )
...
try {
    ...
    if (($wrapperStageExitCode -eq 0) -and (-not $controlIoFailed)) {
        ...
        try {
            ...
            $collectorPath = Join-Path $repoRoot 'src\quant_research\binance_spot_forward_schedule_pit_v5.py'
