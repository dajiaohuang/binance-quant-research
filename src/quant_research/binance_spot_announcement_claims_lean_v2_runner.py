"""Ordinary one-shot publisher for exp_20260826_007 LEAN_R2_2_2 Parent Bootstrap V1.1.
...
VERSION_ROOT = PROCESSED_ROOT / "binance_spot_announcement_claims_v2"
RUNS_ROOT = VERSION_ROOT / "runs"
FINAL_ROOT = RUNS_ROOT / "exp_20260826_007_formal_001"
STAGING_ROOT = FINAL_ROOT.parent / ".exp_20260826_007_formal_001.staging"
CONTROL_ROOT = FINAL_ROOT.parent / ".exp_20260826_007_formal_001.control"

BINDING_SPECS: tuple[tuple[str, str, str], ...] = (
    (
        "--expected-runner-sha256", "runner",
        "src/quant_research/binance_spot_announcement_claims_lean_v2_runner.py",
    ),
    (
    ...
    (
        ...
        "experiments/exp_20260826_007/artifacts/source_contract.json",
    ),
    (
        "--expected-schema-sha256", "schema",
        "experiments/exp_20260826_007/artifacts/schema.json",
    ),
    (
...
def _ensure_existing_directory(path: pathlib.Path) -> None:
    try:
        metadata = path.lstat()
    except OSError as exc:
        raise RunnerError(
            "INFRASTRUCTURE", EXIT_INFRASTRUCTURE,
            f"required parent is unavailable: {path}",
        ) from exc
    ...
    if (
        ...
        raise RunnerError(
            "INFRASTRUCTURE", EXIT_INFRASTRUCTURE,
            f"parent is not a plain directory: {path}",
        )


def _validate_parent_layout(*, shared_parents_must_exist: bool) -> None:
    expected_processed = REPO_ROOT / "data/processed"
    expected_version = expected_processed / (
        "binance_spot_announcement_claims_v2"
    )
    expected_runs = expected_version / "runs"
    expected_final = expected_runs / "exp_20260826_007_formal_001"
    expected_staging = expected_runs / (
        ".exp_20260826_007_formal_001.staging"
    )
    expected_control = expected_runs / (
        ".exp_20260826_007_formal_001.control"
    )
    pairs = (
        (PROCESSED_ROOT, expected_processed),
        (VERSION_ROOT, expected_version),
        (RUNS_ROOT, expected_runs),
    ...
    if any(not _same_path(actual, expected) for actual, expected in pairs):
        raise RunnerError(
            "INFRASTRUCTURE", EXIT_INFRASTRUCTURE,
            "fixed processed output layout mismatch",
        )
    layout_paths = (
        REPO_ROOT, PROCESSED_ROOT, VERSION_ROOT, RUNS_ROOT,
        FINAL_ROOT, STAGING_ROOT, CONTROL_ROOT,
    )
    drives = {_drive(path) for path in layout_paths}
    if len(drives) != 1:
        raise RunnerError(
            "INFRASTRUCTURE", EXIT_INFRASTRUCTURE,
            "processed output layout crosses drives",
        )
    try:
        common = os.path.commonpath(
            [_normalized(REPO_ROOT), *(_normalized(path) for path in layout_paths)]
        )
    except ValueError as exc:
        raise RunnerError(
            "INFRASTRUCTURE", EXIT_INFRASTRUCTURE,
            "processed output layout has no common root",
        ) from exc
    if os.path.normcase(common) != _normalized(REPO_ROOT):
        raise RunnerError(
            "INFRASTRUCTURE", EXIT_INFRASTRUCTURE,
            "processed output layout escapes repository",
        )
    if (
        ...
        or len({_normalized(FINAL_ROOT), _normalized(STAGING_ROOT),
                _normalized(CONTROL_ROOT)}) != 3
    ):
        raise RunnerError(
            "INFRASTRUCTURE", EXIT_INFRASTRUCTURE,
            "formal roots are not distinct same-parent siblings",
        )
    _ensure_existing_directory(REPO_ROOT)
    _ensure_existing_directory(REPO_ROOT / "data")
    _ensure_existing_directory(PROCESSED_ROOT)
    for shared in (VERSION_ROOT, RUNS_ROOT):
        if _lexists(shared):
            _ensure_existing_directory(shared)
        elif shared_parents_must_exist:
            raise RunnerError(
                "INFRASTRUCTURE", EXIT_INFRASTRUCTURE,
                f"shared parent was not created: {shared}",
            )


def _bootstrap_shared_parents() -> None:
    _validate_parent_layout(shared_parents_must_exist=False)
    try:
        ...
        raise RunnerError(
            "INFRASTRUCTURE", EXIT_INFRASTRUCTURE,
            "failed to create fixed shared parents",
        ) from exc
    _validate_parent_layout(shared_parents_must_exist=True)
...
def _run(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        raise RunnerError(
            "INFRASTRUCTURE", EXIT_INFRASTRUCTURE,
            "failed to reserve control directory",
        ) from exc
    ...
    try:
        ...
        raise RunnerError(
            "INTERNAL", EXIT_INTERNAL, "internal failure",
            failure_authorized=True,
        ) from exc
...
def main(argv: Sequence[str] | None = None) -> int:
    try:
        ...
        if exc.failure_authorized and CONTROL_ROOT.exists():
            if (
