...
$experimentId = 'exp_20260826_010'
$runId = 'exp_20260826_010_formal_001'
$controlParent = [System.IO.Path]::GetFullPath(
    (Join-Path $PSScriptRoot '..\..\experiments\exp_20260826_010\formal_control')
)
...
$rawClipboard = $null
$trimmedKey = $null
$clipboardOverwriteMarker='CLEARED_BY_CODEX'
...
try {
    $reservationBytes = $utf8NoBom.GetBytes(
        '{"experiment_id":"exp_20260826_010","run_id":"exp_20260826_010_formal_001"}' + "`n"
    )
...
try {
    ...
    if (($wrapperStageExitCode -eq 0) -and (-not $controlIoFailed)) {
        ...
        try {
            Set-Clipboard -Value $clipboardOverwriteMarker
        }
        catch {
            $preclearFailed = $true
