...
$experimentId = 'exp_20260827_001'
$runId = 'exp_20260827_001_formal_001'
...
$controlParent = [System.IO.Path]::GetFullPath(
    (Join-Path $repoRoot 'experiments\exp_20260827_001\formal_control')
)
...
try {
    $reservationBytes = $utf8NoBom.GetBytes(
        '{"experiment_id":"exp_20260827_001","run_id":"exp_20260827_001_formal_001"}' + "`n"
    )
...
try {
    ...
    if (($wrapperStageExitCode -eq 0) -and (-not $controlIoFailed)) {
        ...
        try {
            if (($rawBytes.Length -ge 3) -and
                ...
                ($rawBytes[2] -eq 0xBF)) {
                throw [System.IO.InvalidDataException]::new('BOM forbidden')
            }
            $textValue = $utf8Strict.GetString($rawBytes)
            $pattern = '\A(?:# Binance read-only API key; local file, never commit\n)?BINANCE_READ_ONLY_API_KEY=([A-Za-z0-9]+)\n?\z'
    ...
    if (($wrapperStageExitCode -eq 0) -and (-not $controlIoFailed)) {
        ...
        try {
            ...
            $collectorPath = Join-Path $repoRoot 'src\quant_research\binance_spot_forward_schedule_pit_v4.py'
    ...
    foreach ($sensitiveName in @('rawBytes', 'textValue', 'keyValue', 'keyMatch', 'buffer')) {
